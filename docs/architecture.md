...
| 项目 | 内容 |
|---|---|
| 版本 / 日期 | v0.9 / 2026-09-24 |
| 状态 | **待复核**：设计提案；已按独立复核第一轮 R-01…R-11、第二轮 N-01…N-08、所有者确认的取舍（§15.5）、需求 v1.7 的判据修订（§10.3）与**需求 v1.8 的范围收缩**（§5.3/§8.4 的撤销）修订，**未被复核者宣布通过**，不据本文宣称已可放行实现；高风险技术验证仍待实现工具执行 |
| 需求基线 | `docs/requirements.md` **v1.8**（原基线 v1.6 的内容提交为 `3187845c`，确认记录 §16.3 提交 `3e2f8a2d`）。两次修订都已写入需求文件：**v1.7** 放宽 DEC-32/DEC-35 的判据（记录见其 **§20.18**）；**v1.8** 把 **DEC-37 改选为选项 ①** —— 不再要求解锁前运行（记录见其 **§20.19**）。本文 v0.9 据此同步：判据按 v1.7、范围按 v1.8 |
| 仓库检查点 | `cc95e37822d859d3bf805788febf80255f9571bf`；`main`，领先本地 `origin/main` 22 个提交；未 fetch；工作区既有 8 个代码文件改动保持不变 |
| 本次范围 | 仅修改 `docs/architecture.md`（v0.9：同步需求 v1.8 的范围收缩 —— 撤销解锁前运行的全部设计）；不实现业务代码、不改需求或规则、不提交或推送；待同步项见 §15.2 |
| DEC-37 交付物 | **v0.9：不再适用** —— 需求 v1.8 已把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复），"解锁前能力边界的核实结论"与其存储/清单改动**随之撤销**（需求 §20.19、本文 §2/§5.3/§8.4） |
...
`AGENTS.md` 的阶段、目录入口及“全部工具缺失”记录已落后于需求 §16.3/§14.6。D-07/D-08 仍需实现前统一，本次不改规则；历史环境记录不等于本轮工具实测。**（2026-09-24 更新：`AGENTS.md` 已按授权改写 —— 阶段与目录入口已对齐，§11 的"环境现状"整段删除、改为"动态探测原则"，**D-08 关闭**；`README.md` 的 fork 段亦已于同日按授权改写，**D-07 关闭**。需求 §17 现无未解决偏差，以上两句记录的是改写前的状态。）**
...
| DD-04 控制状态 | CE 主存 + DE 镜像：双写冲突；全量设置搬到 DE：隐私面大；仅控制状态单主存：小型原生文件读写 | **CE 单主存 + AtomicFile + 单写者**，取消 v0.2 的控制状态双主同步；普通设置仍在 CE，无新库。**v0.9 变更**：存储位置由 DE 改为 CE —— DE 的唯一理由是"解锁前可读"，该理由已随 DEC-37 的修订（需求 v1.8）消失（§5.1、§5） |
...
**~~关键隐私提案 Q-ARCH-01~~（v0.9 撤销）**：该提案原为"解锁前运行"的产物 —— 为了让网络在**首次解锁前**可用，需要把一份最小启动配置快照（可能含 TLS 私钥与接收 PIN）放进**设备加密存储 DE**，并因此由所有者在 v0.7 附加了"仅最小必要快照 / PIN 不裸存、优先 Keystore 绑定 / 解锁后迁移或轮换或清除 / 需威胁模型与审计"四项条件（落地见当时的 §5.3 与 §8.4）。

**需求 v1.8 已把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复）**，解锁前不再需要运行任何东西 ⇒ **该提案及其全部条件、以及为它准备的最小快照、威胁模型、审计事件与两个发布命令一并撤销**。撤销记录：需求文件 **§20.19**。

**撤销时必须留下的一条纪律（不得随提案一起丢掉）**：**"解锁前可访问"不再是一个可以"顺手利用"的位置**。实现中若有人为了别的理由（例如让开机恢复更早）把敏感数据放进 DE 或任何解锁前可读的位置，那是**重新引入本提案已撤销的代价**，必须回到需求侧确认，不得以"技术上更早可用"为由自行推进。
...
| M-STORE ResidentStore | 同上；应用私有 **CE** 文件（v0.9：原为 DE） | 控制状态的唯一持久化写者；不会把 UI 内存当作真相；不运行于 `:quick_tile_service` |
...
输入：平台能力、**用户是否已解锁**（v0.9：解锁状态不再是"能否运行"的门槛，只影响**开机恢复的时点** —— 恢复在首次解锁之后才成立）、Activity 可见性、持久化开关、系统停止证据、启动来源、配置版本、网络可用性。`inactive` 只表示暂时失焦，保留前一可见状态；`detached` 不等于销毁常驻运行时（K-08）。"是否在前台"的具体判据见下方权威来源段（以 `onStart`/`onStop` 为准，**不是** Dart 的 `paused`）。
...
前后台 × 两开关组合必须分别验证：autoStart 只控制开机入口，本次已由用户启动的后台接收不受其关闭影响。**门控只有一个入口**：常规 `BOOT_COMPLETED`（对非 direct-boot-aware 应用，系统本来就在**用户解锁后**投递 —— S-02 与 S-15 记录的平台事实），只有 `backgroundReceiveEnabled && autoStartEnabled && !userStopped` 且配置可用才尝试启动。窗口内重复广播幂等。**v0.9 变更**：不再使用 `LOCKED_BOOT_COMPLETED` 与"解锁兜底"两条入口 —— 解锁前不需要运行（§4.3）。
...
1. 接收开机广播（首次解锁后的常规 `BOOT_COMPLETED`），读取 **CE** 控制记录和系统停止历史；记录入口来源。缺失/损坏/停止证据未知走 §5 的未就绪分支，不先拉起网络再补查。
2. 门控允许才启动独立 connectedDevice 服务，原生先创建实际通知并进入前台，随后初始化应用引擎；不等待 Flutter 初始化才 startForeground。**触发时点**：常规 `BOOT_COMPLETED`，即在用户**完成首次解锁之后**（v0.9；解锁前不启动、不注册任何解锁前入口）。
3. 启动网络时直接**从既有 CE 设置**读取 alias/port/https/receivePin/校验设置与安全上下文（与前台启动同一来源），完成同步状态后才启动发现与 HTTP。CE 此时已可用，因此**不存在**"配置不可用"的解锁前分支；未取到配置**不得**换成默认空 PIN 或临时身份（该红线不变）。
4. **~~`USER_UNLOCKED` 收尾~~（v0.9 撤销）**：原设计用它把设备加密存储中的状态与凭据加密存储对齐；存储已回到 CE、解锁前无副本，该收尾**不再存在**。相应地，**任何系统广播都不得清除"用户已停止"标记** —— 只有用户主动打开应用才可以（§5.1、§4.1 的门控规则）。
5. 用户主动打开时附着现有引擎；若引擎不存在则创建。此路径可解除系统停止门控，按保存的开关恢复。初始化 UI 后才请求缺失权限、显示引导。

K-03 必须覆盖无 Activity 插件注册、RootIsolateToken、FRB、安全上下文、保存目标和用户确认路径；仅看到一条服务通知不是通过。**v0.9 收窄**：K-03 不再需要核实"解锁前"的能力边界（也不需要回答"解锁前是否有可用 Keystore 密钥"）—— 那两项目标随 DEC-37 的修订撤销（§5.3、§8.4）。**保留**"无 Activity 时插件/FRB/安全上下文是否可用"这一条：它仍然必要，因为开机恢复发生在**没有 Activity** 的情形（首次解锁后、用户尚未打开应用）。若在无人交互时无法完成接收确认，**不得**偷偷改为自动接受；该模式的正常握手/取消结果由 K-12 验证。
...
**v0.9：本期只用 CE。** 原设计把控制状态放在 DE，理由是"解锁前要能读"；需求 v1.8 把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复）后该理由消失，存储因此回到 CE。**下面出现"DE"的地方只作为历史或平台事实保留**，实现不得把控制状态或任何敏感材料放进 DE（§2 撤销说明的纪律、§8.4 保留结论 1）。

### 5.1 单写者与数据结构（D）

| 数据 | 字段 / 约束 | 唯一所有者与生命周期 |
|---|---|---|
| ResidentControl v1 | `schema=1`、`revision>=0`、两个 bool 开关、`userStopped`、已处理退出记录标识、最后命令 ID、恢复预算元数据、**`createdBy`/`initializedAt`（仅诊断，R-05）** | 原生 M-STORE；**CE** 的 `resident/control.json`（v0.9：原为 DE），由 AtomicFile 整体提交；生命周期跨进程/重启；不放在 Flutter prefs 再建第二主存。`createdBy` 记录写入来源（首次前台初始化 / 迁移 / 修复），供 K-07/K-10 区分"我们写的默认值"与"读到的他方记录" |
...
- **"记录不存在"的判据定死（R-05）**：CE 控制文件不存在时，**一律走 §4.3 第 1 条的未就绪分支**（不自动启动、启动路径不写任何默认值）；DEC-23 的"缺失键按开启"**只在"用户正常前台启动 + CE 可读 + 确认该键确实不存在"这一条路径上生效**，由前台路径写入初始记录。三种输入因此在设计上走同一条路径：从未初始化 → 未就绪；被外部删除 → 未就绪；损坏/schema 未知 → 未就绪 + 保留诊断（下条）。**"启动路径永不写默认值"是"三条输入走同一路径、且规则不反向"的保证**（若启动路径也会写默认值，"从未初始化"就会被当成"已初始化且为开"，同一条规则随即对同一输入给出相反处理）；**区分来源与事后诊断由 `createdBy`/`initializedAt` 承担**（§5.1），它们**不参与**是否启动的判定，只供 K-07/K-10 的证据区分"我们写的默认值"与"读到的他方记录"。v0.3 的 §4.3 与 §5.2 曾各自读作"未就绪"与"按默认开"，本条消除该歧义，**未改动 DEC-23 本身**（它约束键值语义，不规定启动路径的写入权限）。
...
- 用户主动打开才清除已识别停止标记并更新已处理记录游标，防止同一历史 Stop 在后续恢复时再次阻止运行；不能在 `USER_UNLOCKED` 一类广播中清除（v0.9：本期已不再使用该广播，此处只禁止"用某个系统广播代替用户的显式打开"）。

### 5.3 ~~解锁前最小配置~~（**v0.9 撤销**）

本节原为"解锁前运行"服务：为了在首次解锁前起网络，需要把一份最小启动配置（别名/端口/协议、TLS 安全上下文、接收 PIN/授权策略、必要时保存目标引用）从 CE 派生为 DE 快照，并配套"先失效、再改 CE、再原子发布"的协议与字段白名单。

**需求 v1.8 把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复）后，解锁前不再需要运行任何东西 ⇒ 本节整体撤销**：

- **不再有**解锁前最小配置快照（`resident/boot_config.json` 及其实体）；
- **不再有**发布协议、字段白名单、payload 脱敏要求（撤销理由：载荷本身不存在）；
- **不再有**"身份/PIN/端口变更前先失效快照"的那条前置序列 —— 身份与 PIN 的变更回到常规的 CE 设置变更路径；
- **不再有**保留/清理策略问题（"主动禁用自启后是否清理快照"随快照消失）。

**仍然保留的平台事实（供日后参考，不再作为本期的依赖）**：`PersistenceService` 在 `ls_security_context` 中保存 `StoredSecurityContext`（privateKey/publicKey/certificate/certificateHash），`init.dart` 把它传入 SyncState，ServerService 另读取 alias、port、https、receivePin 与校验设置 —— 也就是说"控制文件可读"与"网络配置可读"本来就是两件事（F）。这条事实本期只用于说明**开机恢复路径必须等 CE 可用**（即首次解锁之后），不再用于设计 DE 快照。

**撤销记录**：需求文件 §20.19；本节的四个字段分组、发布协议与三条约束**随本节一并失效**，不得被实现阶段"捡回来"使用（见 §2 撤销说明里的纪律）。
...
| ~~`residentPublishBootConfig` / `residentInvalidateBootConfig`~~ | — | **v0.9 撤销**：它们是"解锁前最小配置快照"的载体（§5.3），快照本身已随 DEC-37 的修订取消，命令不再存在。身份/PIN/端口的变更回到常规的 CE 设置路径 |
| `residentReadBootstrap` | 当前引擎代次；由原生建立的启动上下文 | **v0.9 改写**：不再返回解锁前快照。开机恢复发生在**首次解锁之后**，此时 CE 已可用 ⇒ 启动路径直接**从既有 CE 设置读取** alias/port/https/receivePin/校验设置（与前台启动同一来源），不存在"配置不可用"的解锁前分支。仍不得整包打印敏感字段 |
...
输入检查：拒绝未知 apiVersion、错误类型、负 revision、未知字段/动作；方法调用方不允许传任意文件路径、组件类名或“来自用户打开”的伪造标记。**（v0.9）敏感载荷通道已随 BootConfig 撤销而不存在**：见 §5.3 与 §8.4 的撤销说明。通知停止使用显式且不可变 PendingIntent，固定内部 action/目标，不暴露通用执行入口。
...
- 接收策略、PIN、保存目录与 SAF 权限在前台/后台保持一致（v0.9：删去"解锁前"——本期不再有该窗口；开机恢复发生在解锁后，与前台同一来源）。后台缺 Activity 不能弹目录选择；记录能力不可用并按既有拒绝/错误路径应答，不能自动接受、换目录、忽略 PIN。若这导致既有功能或 AC-10 无法满足，列为 K-03 风险，不伪称降级成功。
...
**v0.9：不再声明 `directBootAware`。** 原设计让接收者与常驻服务在首次解锁前可用；需求 v1.8 撤销解锁前运行后，它们**只需要在解锁后工作**，因此不需要该属性，也不需要 `LOCKED_BOOT_COMPLETED`。`stopWithTask=false` 仅表示应用不主动随任务移除停服务，不承诺 ROM 必然保留。未初始化运行时前先完成原生 FGS 晋升；符合类型权限不等于自动豁免全部后台启动限制（S-03/S-04）。
...
| 组件 | 关键属性 | 说明 |
|---|---|---|
| `BootReceiver`（新增） | `exported=false`（**待核实**）、`enabled=true`、不指定 `android:process`（默认进程）；**v0.9：不声明 `directBootAware`** | 只监听 `BOOT_COMPLETED`（首次解锁后投递）；只在门控通过时启动常驻服务（§4.1）。`exported=false` 的依据是"非导出接收者仍可接收**系统**发送的广播"（S-13）—— 该结论本轮经检索获得、**未能直连核对官方页面**，故列为待核实 |
...
| 快捷磁贴服务 | 现状 `android:process=":quick_tile_service"`（F：应用清单） | M-STORE 不运行于该进程，不得由磁贴直接写 CE 控制文件（§5.1） |

该表是**契约草案**：最终以合并清单复核为准（K-13）。若实现需要表外组件、action 或属性，**先回到本表补条目**，不静默新增。

**启动路径必须被断言（N-04 保留，v0.9 改写）**：常规 `BOOT_COMPLETED` 是否真的送达新增接收者，是"开机恢复"能否成立的前提；**它一旦不成立，外部表现是"静默不启动"**，而不是报错。因此 K-08 与 K-13 的通过标准都包含"重启并完成首次解锁后 `BootReceiver` 确实被调用"这一条（见 §10.4），不接受"没有异常"当作证据。**v0.9 缩减**：原先还要断言"**未解锁**重启后接收者被调用"（那是 DEC-37 选项 ② 的前提），该断言已随修订取消。

### 8.3 配置与密钥管理

复用既有端口/协议/接口筛选/证书管理入口，保活模块不设第二套网络配置界面。**（v0.9）** 原先那句"在 Q-ARCH-01 的条件被满足之前不得实施敏感 BootConfig 迁移"**随快照撤销而失效**：本期不存在解锁前快照，也就不存在"迁移"。身份与 PIN 仍只存在于 CE 的既有设置中，不产生派生副本（§5.3 撤销说明、§8.4 保留结论 2）。不能复制整个 SharedPreferences 文件以简化初始化。

**~~BootConfig 的通道约束（D，R-06）~~（v0.9 撤销）**：该约束原为"解锁前最小配置快照"服务（发布/失效两个命令、字段白名单、载荷不进日志/回执/诊断）。快照与两个命令已随 DEC-37 的修订取消（§5.3、§6.1），因此**不存在这条敏感载荷通道**。**保留的纪律**：若日后确需新增任何"把敏感材料送往后端存储"的通道，必须先回到需求侧确认，并重新补上"字段白名单 + 不进日志/回执/诊断/附件"的约束 —— 不得因为这一段被撤销就认为该纪律失效。
...
### 8.4 ~~解锁前快照的威胁模型与审计~~（**v0.9 撤销**）

本节原为 Q-ARCH-01 的条件之一（所有者 v0.7 要求"记录威胁模型与审计"），分析对象是"放进 DE 的解锁前最小快照"。**需求 v1.8 撤销解锁前运行后，快照不存在，本节随之撤销** —— 五类对手的对照表、五条审计要求、以及"解锁前是否存在可用 Keystore 密钥"这一留给 K-03 的平台问题，**全部不再需要**（撤销记录：需求 §20.19）。

**撤销时保留两条对本期仍然有效的结论**（它们不依赖快照，属于通用纪律）：

1. **不要把敏感材料放进"解锁前可访问"的位置**。这条在 v0.9 之后**更容易满足**（不再有任何理由这么做），但纪律本身要留着：DE 目录、解锁前可读的任何文件、以及"为了更早可用"的变通做法，都属于**扩大暴露窗口**，须先回到需求侧确认（§2 撤销说明）。
2. **最小化优先于保护手段**：字段越少越好、能不进持久化就不进。本期网络只在首次解锁后启动，因此 **TLS 身份与接收 PIN 直接留在 CE**，不需要任何派生副本 —— 这正是"最小化"的直接结果。
...
- L1 每实现任务执行需求 M-01…M-07 和专项断言；非 Android 构建为发布门槛。AC-03/04/05 长跑、真实路由器和部分 ROM 行为属于 L2；**组播与"停止组合"先实测能否 L1，不预判一律 L2**（v0.9：原句还列了"解锁前"，该项已不在范围内）。
...
| REQ-006 | M-NATIVE/STORE/NET | TC-06；Android L1（v0.9：不再需要 Direct Boot 环境） | **AC-10：完成首次解锁后**发现且连接（双判据），先 C 后 A，采样起点为"完成解锁"；AC-13 自启关闭不恢复；系统 Stop→重启→（解锁后也）不恢复 |
...
停止组合若最终只能 L2，保留静态门控检查及"解锁后但未打开应用不恢复"的 L1 替代证据；替代证据不等于通过。（v0.9：原句末尾的"不等于解锁前通过"已无对象 —— 解锁前不在承诺内。）

**需求 §14.2.1 最小覆盖集的落位核对（R-09）**：该节 9 行场景与上表的对应关系为 —— 四种开关组合 → TC-01；首次启动默认值与引导（含"无法检测"） → TC-09；启动被拒回滚 → TC-01 的失败分支；**服务已启动但监听失败 → TC-02（本次补入 DEC-22 判据）**；关闭通知渠道 → TC-03b；两通知并存 → TC-03/TC-11；关闭后无残留 → TC-08（AC-12，B 类）；**系统"活动应用"停止之后的行为 → TC-03（本次补入 DEC-19 负向用例与 30 分钟窗口）**；"停止 → 重启 → 解锁后（也不恢复）"组合 → TC-06（正负两条都在）。9 行均有落位；落位只证明覆盖声明存在，不证明用例已执行。
...
|---|---|---|---|
| K-01 | API36 专用 AVD：`adb shell getprop ro.crypto.type`、重启后观察解锁状态与 `BOOT_COMPLETED` 的投递时点（`logcat` 看接收者被调用的时刻） | **v0.9 目标改写**：确认"`BOOT_COMPLETED` 在首次解锁后才投递"这一前提（本设计据此把开机恢复定位在解锁后）；不再需要验证"未解锁可运行" | 换非生产测试镜像；不为验证转换加密/擦除现有设备；无法自动化归 L2 |
| K-02 | 重启 → **完成首次解锁** → 独立客户端扫描/握手；记录重启、解锁、接收者被调用的时刻 | **v0.9 改写**：**不打开 app**、解锁后，AC-10 的双判据在 30min 窗口内成立，采样起点为"完成解锁"（§10.2 的 TC-06）；端口转发未替代发现链路 | 先修测试拓扑；自动化不可达则 L2 并如实记录；产品不达则回需求取舍 |
| K-03 | 最小后台宿主原型：**无 Activity** 初始化 Flutter/FRB/isolate（开机恢复时用户尚未打开应用），字段读取清单，TLS/PIN/保存策略逐项验证 | 保持同一身份与既有安全/接收行为；无误读 CE、无后台弹窗；无 Activity 时能完成启动与正常握手/清理；不能只起空服务；**PIN/私钥等敏感材料只从 CE 读取，不产生任何派生副本** | 先在同一方案内减少 UI 依赖；若"无 Activity 时插件/FRB/安全上下文不可用"成立 ⇒ 该路径不成立，回需求取舍（不得静默降级）；**v0.9 已撤销**"解锁前能力边界"与"解锁前 Keystore 可用性"两项（§5.3、§8.4） |
...
| K-07 | Stop 后立即 reboot，**完成首次解锁**（v0.9：原为"全程未解锁"）；正常退出/崩溃对照；读取 ApplicationExitInfo | 后台入口可可靠识别停止且跨重启保持，主动打开后不会被旧记录再次阻止 | 若历史不可读或不可区分，明确技术不满足，提交需求取舍；不加互守进程，不默认未停止 |
...
| K-10 | 原生文件（CE 的 `resident/control.json`）写入中断/损坏/schema 未知、控制记录迁移注入失败 | 只读到完整旧/新记录；关闭不会被旧镜像覆盖；升级保留选择 | 降为显式修复状态，记录未达成；修存储协议再测，不抹数据强制默认开启。**v0.9**：不再涉及 BootConfig 迁移（§5.3 撤销） |
...
| S-02 [Android：Direct Boot](https://developer.android.com/privacy-and-security/direct-boot) | CE/DE 边界、directBootAware、解锁广播；最小数据原则。**v0.9：本期不再依赖该页面支撑的任何设计** —— 解锁前运行已撤销；仅保留一条**平台事实**用作反面依据：**对非 direct-boot-aware 应用，`BOOT_COMPLETED` 在用户解锁后投递**（这正是"开机恢复落在解锁后"的依据） |
...
| S-15 [Android：Direct Boot](https://developer.android.com/privacy-and-security/direct-boot)（与 S-02 同一页面）与 Keystore 可用性 | v0.7 复核 Q-ARCH-01 的条件时查阅：Direct Boot / DE 存储自 **API 24** 起存在；`setUnlockedDeviceRequired` 自 **API 28** 起存在。**"解锁前是否存在可用的 Keystore 密钥"当时未取得结论**（`developer.android.com` 直连不可用）。**v0.9：该问题已随解锁前运行的撤销而失去对象**，不再作为 K-03 的核实项，本行只作历史保留 |
...
| `grep -cE '^\| (REQ-[0-9]{3}\|NFR-[0-9]{3}) \| ' docs/architecture.md` | `21`；逐项列举后 `REQ-001…REQ-011` 与 `NFR-001…NFR-010` 各自齐备、无重复 | 只证明条目存在。**v0.9 复核**：需求范围收缩**不改变**这 21 行的条数 —— 改的是 REQ-006 的判据内容与 DEC-37 的行文，不是增删条目 |
...
| v0.9 | 2026-09-24 | **同步需求 v1.8 的范围收缩**（**状态保持"待复核"**）：需求把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复）⇒ 本文**撤销解锁前运行的全部设计** —— §2 的 Q-ARCH-01 提案、§5.3 的解锁前最小配置快照、§6.1 的两个 BootConfig 命令、§8.3 的载荷通道约束、§8.4 的威胁模型与审计、§9.2 的 `bootconfig.*` 事件、`directBootAware` + `LOCKED_BOOT_COMPLETED` + `USER_UNLOCKED`（§4.1/§4.3/§8.2）；**控制状态由 DE 回到 CE**（DD-04、M-STORE、§5、§5.1/§5.2）；§4.3 的开机恢复明确落在**首次解锁之后**且直接从 CE 读配置；K-01/02/03/07/10 的目标改写；§10.2 的 TC-06、§14.2 的 Q-ARCH-01 行、§14.3 的两条 P1 风险、§14.4 的交付物 ④⑤、§15.2/§15.4/§15.5/§15.6 的记录同步；头部需求基线改 v1.8 |

## 14. 结尾：关键决策、待确认项、风险与计划准入

### 14.1 关键决策

推荐沿用既有技术栈与协议，采用独立Kotlin常驻服务、同一应用网络引擎复用、原生CE控制状态单主存（v0.9：原为DE）、单控制器串行收敛、事件驱动有界恢复、本地诊断与APK私有侧载。先不增加依赖、数据库、后端或Alarm/Job兜底。DD-04、DD-05及敏感BootConfig是影响较大的设计选择，其代价和验证条件已展开，不因文档写完而视为用户确认。

### 14.2 待确认项（集中处理，不重复询问已定需求）

| 项 | 推荐 / 需要的取舍 | 时机与不依赖它的工作 |
|---|---|---|
| ~~Q-ARCH-01 解锁前敏感数据~~ **已撤销（v0.9）** | 需求 v1.8 把 DEC-37 改选为选项 ①（只要求"首次解锁后"恢复）后，**解锁前不再需要运行** ⇒ 最小启动配置快照、其威胁模型与审计、两个发布命令、`bootconfig.*` 事件、以及"解锁前是否有可用 Keystore 密钥"**全部失去对象**（§2、§5.3、§8.4） | **已关闭，无需落地**。原先挂起的 K-03 核实项一并取消。**保留的纪律**：不得因"更早可用"把敏感材料放进解锁前可访问的位置（§2 撤销说明） |
...
| 级别 | 风险 / 影响 | 处置 |
|---|---|---|
| P1 技术未闭合 | 系统 Stop 跨重启的证据不可取，DEC-19 与开机恢复的组合可能无法保证 | K-07 优先；失败带证据回需求，不默认为允许恢复。**v0.9**：原先还含"解锁前证据不可取"，随 DEC-37 的修订取消 |
| ~~P1 技术与隐私~~ **已收窄（v0.9）** | 原风险是"Flutter/插件/目录不能 Direct Boot 运行，或最小配置仍必须扩大敏感数据暴露面"。**解锁前运行被撤销后，两条都不再成立**：路径不再经过 Direct Boot，也不存在解锁前快照。**残留的相邻风险**收窄为"无 Activity 时插件/FRB/安全上下文不可用"（开机恢复时用户尚未打开应用） | **K-03** 覆盖该残留项；不能降 TLS/PIN 或替换身份 |
...
> **设计阶段交付物的状态（如实登记，R-08；v0.9：由五项缩为三项）**
>
> | 交付物 | 状态 | 位置 |
> |---|---|---|
...
> | ~~④ 解锁前能力边界的核实结论（DEC-37，含存储方案与 `directBootAware` 清单改动）~~ | **v0.9 撤销**：需求 v1.8 改选选项 ① 后不再是交付物（需求 §20.19） | — |
> | ~~⑤ 解锁前快照的威胁模型与审计要求（Q-ARCH-01 的条件之一）~~ | **v0.9 撤销**：快照不存在，分析对象消失（§8.4） | — |
>
> **v0.9 更新**：上表已从五项缩为三项 —— ④ 与 ⑤ 随"解锁前运行"的撤销一并取消。原先那句"不得改成仅解锁后运行"的禁令**随之失效**：**"仅解锁后运行"现在就是需求本身**（DEC-37 的 v1.8 修订）。第 3 条仍适用于 ①（服务端生命周期状态机，已闭合）、②（探测规程，唯余等价性前置）与 ③（追踪矩阵）。
...
3. **不能据此直接放行依赖它们的业务实现**：K-03（无 Activity 时的宿主可行性）、K-07/08/10/12 等关键结果未回填前，不把对应方案标为技术已定，也不把"只有计划"当作结论。失败先更新设计或提交需求取舍，再安排依赖任务。**v0.9**：原句还要求"设计阶段交付解锁前能力结论"，该项已随 DEC-37 的修订撤销（§14.4）。
...
| **R-06** BootConfig 写入路径未定义、与单写者不对齐 | **成立**。§5.1 声称 M-STORE 是唯一持久化写者，但 BootConfig 内容来自 Dart 持有的 CE，契约表里没有对应的发布命令 | §6.1 新增 `residentPublishBootConfig`（含字段白名单）与 `residentInvalidateBootConfig`；§6.1 输入检查；§5.1 的 BootConfig 行写明 actor；§5.3 发布协议补 payload 白名单、脱敏、写入路径唯一 | **采纳**（按复核建议的最小修正）。**v0.9：整条随解锁前快照一并撤销** —— 快照与两个命令已取消（§5.3、§6.1、§8.3），§15.2 的对应待同步项亦已撤销。当时记的纪律（敏感载荷须有白名单 + 不进日志/回执/诊断）在"日后若新增此类通道"时仍然适用 |
...
| ~~Q-ARCH-01 条件的落地结论~~ **v0.9 撤销** | — | 不再需要 | 解锁前运行已撤销，该条件与其触发分支一并消失（需求 §20.19） |
...
| ~~D-07 / D-08（`README.md` fork 段过期、`AGENTS.md` §11 环境记录过期）~~ **均已关闭（2026-09-24）** | `README.md` / `AGENTS.md` / `docs/requirements.md` | 不再需要 | 经所有者授权分别改写：`AGENTS.md` §11 不再记录环境（改为动态探测原则）；`README.md` 的 fork 段改为指向需求文档并写明本 fork 的协作规则位置。记录见需求 §17 的 D-07、D-08 两行 |
...
**仍待复核者判定（v0.5 新增；v0.6/v0.9 更新）**：N-01 引出的 Q-ARCH-03 **已由所有者决定为"终止"**（§15.5），不再是开放项；**Q-ARCH-01 已于 v0.9 整条撤销**（解锁前运行取消，快照失去对象）。**当前仍开放的判定只剩实测类**：**N-04 的 `exported` 取值与启动路径被调用**、**N-05 的可见性边界**（两者都靠 K-08/K-13 回填），以及 **DEC-35 的"等价性"**（§10.3 前置 1）。
...
| 启动路径被调用（N-04） | **v0.9 改写**：重启 → **完成首次解锁** → `adb shell dumpsys activity services` + `logcat` 确认 `BootReceiver` 被调用。~~原命令用 `LOCKED_BOOT_COMPLETED` 模拟未解锁重启~~ —— 该情形已不在承诺内（需求 §20.19），也不再据此断言 | K-01 先行 | K-08 / K-13 |
...
|---|---|---|---|
| ~~**Q-ARCH-01** 解锁前敏感数据~~ | v0.7 曾"有条件确认"并落地为 §5.3 第 1–7 条、§8.4、`bootconfig.*` 事件、K-03 断言 | **v0.9 全部撤销**：§2、§5.3、§8.4、§6.1（两个命令）、§8.3、§9.2、K-03/§14.4 已改为撤销态 | **已关闭，无需落地**。撤销原因是需求 v1.8 把 DEC-37 改选为选项 ① —— 解锁前不需要运行，快照失去对象（需求 §20.19）。**保留纪律**：不得为"更早可用"把敏感材料放进解锁前可访问的位置（§2） |
...
| §2 / §5.3 / §8.4 的撤销、CE 存储、开机恢复落在解锁后、K-01/02/03/07/10 的目标改写（**v0.9**） | **DEC-37 的 v1.8 修订**（选项 ② → ①）。其中"以 `BOOT_COMPLETED`（首次解锁后投递）为触发""不用 DE""未达窗口的缺口是接受的代价、禁止实现阶段补上"三项口径**已于 2026-09-24 由所有者逐项确认** | `docs/requirements.md` **§20.19**（含三项确认表）与 §16.1 的 DEC-37 行 |
