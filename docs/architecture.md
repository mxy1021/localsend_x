...
`AGENTS.md` 的阶段、目录入口及“全部工具缺失”记录已落后于需求 §16.3/§14.6。D-07/D-08 仍需实现前统一，本次不改规则；历史环境记录不等于本轮工具实测。**（2026-09-24 更新：`AGENTS.md` 已按授权改写 —— 阶段与目录入口已对齐，§11 的"环境现状"整段删除、改为"动态探测原则"，**D-08 关闭**；仍未一致的只剩 **D-07**（`README.md`），处置层级仍为"进入实现前必须统一"。）**
...
| D-07（`README.md` fork 段过期） | `README.md` | 需单独授权 | 沿用 v0.3 的记录。**D-08 已于 2026-09-24 按授权关闭**：`AGENTS.md` §11 不再记录环境（改为动态探测原则），记录见需求 §17 的 D-08 行 |
