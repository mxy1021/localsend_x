...
- 现状：**需求基线已确认、架构设计仍在复核中**；本文件只覆盖通用约定，项目规则按上表分项补全。开发与测试要求在进入相应阶段后才适用。
- 本文档内的占位与回填时机：

| 待补全内容 | 标记 | 补全条件 |
|---|---|---|
| 业务目标与需求范围 → 项目规则 | **需求侧已满足：可补** | 需求基线已由仓库所有者确认（确认记录见 `docs/requirements.md` 头部与 §16.3） |
| 技术与目录约定 → 项目规则 | **架构确认后补全** | `docs/architecture.md` 的状态行不再是"待复核"后 |
| 安装 / 测试 / 构建命令及其可用性 | **工程初始化已完成，实测待补** | 工程初始化**已完成**（逐项证据见 `docs/requirements.md` §14.6）；**实测**待按 M-01…M-07 跑通后回填 |

- 本文件的补全**只以上表各行的补全条件为准，不引用外部步骤编号**（仓库内没有步骤计划文档，避免出现无出处的引用）。
...
- **项目目标：以 `docs/requirements.md` 为唯一权威**（已确认为需求基线，版本与确认记录见该文件头部与 §16.3）：本轮范围为 Android 端后台保活与开机自启，目标、范围与验收判据的正文在该文件，**本文件不复述、不代定**。**该文件未确认的需求不得据此设计或实现**；新需求按 §9 第 3 段记录，不实现。
...
| 需求文档 | `docs/requirements.md` | **已存在**（2026-09-23 创建） | 每次任务前 |
| 架构 / 设计文档 | `docs/architecture.md` | **已存在**（v0.9 / 2026-09-24，状态"待复核"） | 需要了解方案或做设计时 |
| 交接文档 | `docs/handoff.md` | **后续创建**（当前不存在） | 存在时再读；模型交接按 §7 写入 |
...
3. 读本文件、`CLAUDE.md` 及**已存在**的任务相关文档（需求见 `docs/requirements.md`，设计见 `docs/architecture.md`）；确实尚未生成的文档（如交接文档）**不要求读取**，只如实说明其缺失。文档是否存在**以 §1 的表格为准**，不得凭印象判定某文档"尚未生成"。
...
- **安装 / 测试 / 构建命令：回填条件只满足前半**。**工程初始化已完成**（逐项证据见 `docs/requirements.md` §14.6），但**命令一个都还没实测过** ⇒ **一律按"不可用"处理**，写"未验证 + 原因"，直到按 §5/§9 附上真实输出为止（§11）。工具链是否就位、网络是否可达，由**执行时的动态探测**判定 —— 本文件不记录环境现状（§11）。
...
- **需求 ID 格式**：功能性需求 `REQ-<三位序号>`，非功能需求 `NFR-<三位序号>`（2026-09-23 由仓库所有者确认，取代原先的 `REQ-<域>-<三位序号>` 分段格式）。**需求条目正文由 `docs/requirements.md` 维护（已生成）；不得凭记忆编造需求条目或 ID。**
...
- **模型交接载体**是仓库内可见的提交信息或交接文档，**不依赖聊天记录**。交接内容至少含：当前分支、需求 / 任务 ID 及状态、已执行命令与真实输出、未解决项、下一步、未验证声明。交接文档路径定为 **`docs/handoff.md`**；该文件尚未创建，**创建前的交接内容写进提交信息**。
...
- 已知文档偏差（**暂不修正**，勿据此行事）：`README.md` fork 段仍写"本 fork 未对这两份文档做改动"（已过期），并写「目标用户 / 要解决的问题 **待确认**」（与已确认的需求基线冲突 —— 见 `docs/requirements.md` §17 的 D-07，其处置层级为**进入实现前必须统一**）；上半部分 FOSS 节称 `lib/provider/purchase_provider.dart` 带 `FOSS_REMOVE` 标记（实为整文件删除）。**D-08（本文件 §11 环境记录过期）已随需求 v1.9 的环境记录同步关闭**。
...
**已确认基线（2026-09-23，确认人：仓库所有者）**：

- **基线文档**：`AGENTS.md`（本文件，启动版）+ `CLAUDE.md`（独立一行 `@AGENTS.md` 导入本文件）。
- **基线版本**：`fe5e5929` —— 本次确认针对该提交的 `AGENTS.md` 内容与未改动的 `CLAUDE.md`（后者自上游基线 `230fb692` 起零改动）。
- **本期范围**：**仅启动阶段的协作规则**（该次确认的范围；此后需求基线已确认，项目目标与范围以 `docs/requirements.md` 为准 —— 见 §1）。
- **取舍 / 延期项**：无。
- **已获准进入**：整理需求文档（你的确认中称"第 3 步"；仓库内暂无完整步骤计划文档，故 §0 的补全条件不引用该编号）。
- **确认粒度**：普通实现与修复**无需重复确认**；**新增需求、关键方案变更、超出既有授权的操作**（§8 清单）需单独说明。

**动态探测原则（本文件不记录本机环境）**：

- **不在本文件固化本机环境**：不写工具版本号、安装路径、环境变量取值、网络或设备状态、探测输出。环境事实的逐项证据只存在于 `docs/requirements.md` §14.6 —— 那是**工程初始化那一步的记录**，**不是"当前环境状态"**，不得当实时事实引用。
- **执行任何构建、测试前必须先动态探测环境**（例如 `git --version`、`fvm flutter doctor`、`java -version`，按需选择），**以探测结果为准**。
- **禁止依赖过期的环境快照**。若探测发现网络不通、依赖不可达或模拟器未运行，**必须立即终止任务并报告**（写明探测命令与真实输出），**禁止盲目重试**；恢复条件（启用代理、启动模拟器）会改变本机状态，属 §8 范围，**需单独授权**。
- **命令只有真实执行通过后才能标记为"已验证"**，未跑通一律写"未验证 + 原因"（与 §4、§9 第 2 段一致）。

**待补全项汇总**：**见 §0 的补全点表**。完整回填条件同 §0，不引用外部步骤编号。
