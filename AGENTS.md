...
| 待补全内容 | 标记 | 补全时机 |
|---|---|---|
| 业务目标、技术栈、目录约定 | **需求与架构确认后补全** | 设计确认后 |
| 安装 / 测试 / 构建命令及其可用性 | **工程初始化后验证并回填** | 工程初始化后 |
| 本文件按确认后的设计整体补全 | —— | **第 6A 步** |
| 回填真实命令、去掉未验证标记 | —— | **第 9 步** |
...
- **安装 / 测试 / 构建命令：工程初始化后验证并回填**。当前所有构建、测试、运行命令**不可用**（实测见 §11），仅 `git` 只读命令（`status` / `log` / `diff`）可用。
- **只有实际跑通的命令才能标为"可用"**；未跑通、未安装的一律不标可用。**禁止声称未实际运行的测试、构建或人工验收通过**；无法验证时写"未验证 + 原因"。
...
**环境现状**（实测 2026-09-23，方式：`cmd //c where`、环境变量、常见安装目录限定深度搜索）：`git` 可用；`fvm` / `flutter` / `dart` / `cargo` / `rustc` / `rustup` / `flutter_rust_bridge_codegen` / JDK（`java`）/ Android SDK 均**未找到**，`adb` 只有第三方软件附带的散落副本，`support/submodules/flutter` 子模块未初始化 ⇒ **当前无法执行任何构建、测试或设备验证**（限定范围搜索，不排除未覆盖路径下已安装工具）。

**待补全项汇总**：业务目标 / 技术栈 / 目录约定 → **需求与架构确认后补全**；安装 / 测试 / 构建命令 → **工程初始化后验证并回填**；本文件整体补全 → **第 6A 步**（按确认后的设计）；命令回填 → **第 9 步**。
