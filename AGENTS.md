...
- 已知文档偏差：`README.md` 的 fork 段（"未对这两份文档做改动"与「目标用户 / 要解决的问题 **待确认**」）**已于 2026-09-24 按授权修正，D-07 关闭**；**D-08（本文件 §11 环境记录过期）亦已关闭** —— 两者记录见 `docs/requirements.md` §17。**仍存在的一处**：上半部分 FOSS 节称 `lib/provider/purchase_provider.dart` 带 `FOSS_REMOVE` 标记（实为整文件删除）—— 属上游原文，**暂不修正，勿据此行事**。
